...
namespace MarcusRunge.Mopr.Workbench.Services.Application.Test.Identity
{
    public sealed class UserProvisioningServiceTests
    {
        ...
        public async Task ProvisionAsync_WhenValuesContainWhitespace_NormalizesBeforePersistence()
        {
            ...
            context.UserRepository.Verify(x => x.AddAsync(It.Is<User>(user => user.FirstName == "Marcus Alexander" && user.LastName == "Runge" && user.ShortName == "M R"), TestContext.Current.CancellationToken), Times.Once);
        }
        ...
        public async Task ProvisionAsync_WhenCreationFailsWithoutConcurrentUser_ReturnsFailed()
        {
            ...
            context.UserRepository.Setup(x => x.GetByLoginNameAsync(LoginName, TestContext.Current.CancellationToken)).ReturnsAsync(() =>
            {
                lookupCount++;
                return null;
            });
